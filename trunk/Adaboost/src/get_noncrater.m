function get_noncrater(test,classf)
...
for img_n = 1:param.test.sz_dataset
    ...
    % just the ones that had negative feedback:Siyi Liu
    whole_index=1:(last_testset-first_testset+1);
    neg_ind=setdiff(whole_index,ind);
    
    
    HH_image = HH(first_testset:last_testset);
    
    % again, the ones that had positive feedback
    HH_image = HH_image(neg_ind);
    locs = locs(neg_ind,:);
    index = index(neg_ind);
    features = features(neg_ind,:);
    ...
    non_craters(img_n).locs = locs;
    non_craters(img_n).index = index;
    non_craters(img_n).x = features;
    non_craters(img_n).hhimage=HH_image';
...
if ~isdir(param.non_craters.path)
    mkdir(param.non_craters.path)
end

% register filename with timestamp
cratersset_filename = [param.non_craters.path param.time '.mat'];

save(cratersset_filename,'non_craters');
